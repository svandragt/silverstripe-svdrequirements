...
class SvdRequirements extends Requirements
{
     ...
     */
    public static function combine($files, $media = null)
    {
        if (is_string($files)) {
            $files = array($files);
        }
     ...
     */
    private static function backtrace_class()
    {
     ...
     */
    private static function extension_from_filename($file)
    {
        $pi  = pathinfo($file);
        $ext = $pi['extension'];
     ...
     */
    private static function populate_assets($files)
    {
        $assets = array();
